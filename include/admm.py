...
def deconvtvl2(image, H, mu):
    ...
    eigDtD = np.abs(beta[0] * np.fft.fftn(np.array([[1, -1]]), (rows, cols))) ** 2 + np.abs(
        beta[1] * np.fft.fftn(np.array([[1], [-1]]), (rows, cols))) ** 2
    ...
    rnorm = np.sqrt(
        np.linalg.norm(Df1.flatten()) ** 2 + np.linalg.norm(Df2.flatten()) ** 2 + np.linalg.norm(Df3.flatten()) ** 2)
    ...
    for itr in range(max_itr):
        ...
        rnorm_old = rnorm
        rnorm = np.sqrt(np.linalg.norm(Df1.flatten() - u1.flatten()) ** 2 + np.linalg.norm(
            Df2.flatten() - u2.flatten()) ** 2 + np.linalg.norm(Df3.flatten() - u3.flatten()) ** 2)
